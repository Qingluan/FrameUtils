...
func (self *TaskConfig) SimeplUI(w http.ResponseWriter, r *http.Request) {
	...
	function showDetail(id){
		$.post("/task/v1/api", JSON.stringify({
			...
			$("#detailFrame").contents().find("body").append("<pre>" + data.log+"</pre>")
	...
	function newTask(){ 
		$("#showModal").modal();
		  ...
		  `,
			...
			JQuery:     web.Jquery,
			BJQuery:    web.BootstrapJquery,
			UploadJS:   web.UploadJS,
			UploadHTML: web.UploadHTML,
			UploadCSS:  web.UploadCSS,
		}
		t1, _ := template.New("base").Parse(TEMP)
		// template.Must(template.New("upload").Parse(onePage.UploadHTML)).Execute(struct {
		// 	Action string
		// }{Action: "/task/v1/taskfile"})
		...
		// onePage.ServerNum = strings.Join()
