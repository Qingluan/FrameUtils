...
function newTask(){
	$("#showModal").modal();
	  ...
	  `,
		BJs:    web.BootstrapJS,
		BCss:   web.BootstrapCSS,
		JQuery: web.BootstrapJquery,
	}
)
...
func (self *TaskConfig) SimeplUI(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		t1, _ := template.New("base").Parse(TEMP)
		log := self.GetMyState()
		TEMPStruct.ServerIP = utils.GetLocalIP()
		TaskNum, _ := log["task"]
		TEMPStruct.ServerNum = fmt.Sprintf("%d", len(self.Others))
		ReadyNum, _ := log["wait"]
		...
		if fs, err := ioutil.ReadDir(self.LogPath()); err == nil {
			...
			paths := []string{}
			for _, f := range fs {
				TEMPStruct.Logs = append(TEMPStruct.Logs, LogUI{
					ID:       f.Name(),
		...
		TEMPStruct.LogRoot = self.LogPath()
		// TEMPStruct.ServerNum = strings.Join()
		TEMPStruct.TaskNum = TaskNum.(string)
		TEMPStruct.ReadyNum = ReadyNum.(string)
		TEMPStruct.RunningNum = RunningNum.(string)
		TEMPStruct.LogsNum = LogNum.(string)
		TEMPStruct.ErrNum = ErrNum.(string)
		// "running": fmt.Sprintf("%d", len(task.RunningChannel)),
		// "wait":    fmt.Sprintf("%d", len(task.WaitChannel)),
		// "config":  string(buf),
		// "logs":    string(buf2),
		t1.Execute(w, TEMPStruct)
		// t1
		// t1 := template.Must()
	}
}
